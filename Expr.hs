...
eval vars (Abs x) = do {q <-  (eval vars x) ; Just (abs q) }
eval vars (Mod x y) = do {q <-  (eval vars x) ; p <-  (eval vars y) ; Just (mod q p ) } 
eval vars (Power x y) = do {q <-  (eval vars x) ; p <-  (eval vars y) ; Just (q ^ p) }
